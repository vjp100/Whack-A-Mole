...
entity graphic_renderer is
    Port (
        ...
        -- NEW: hit animation signals from mole_generator
        hit_flash_active : in std_logic;
        hit_flash_hole   : in std_logic_vector(3 downto 0);
...
architecture Behavioral of graphic_renderer is
    ...
    signal hit_flash_hole_int : integer range 0 to 15 := 0;
    ...
    signal hit_flash_hole_d     : integer range 0 to 15 := 0;
    signal hit_flash_active_d   : std_logic := '0';
...
begin
    ...
    hit_flash_hole_int <= to_integer(unsigned(hit_flash_hole));
    ...
    begin
        if rising_edge(clk) then
            ...
            hit_flash_hole_d     <= hit_flash_hole_int;
            hit_flash_active_d   <= hit_flash_active;
    ...
    begin
        if ((x_int_d / 32) mod 2 = 1) and ((y_int_d / 16) mod 2 = 1) then
            grass_rgb <= x"473"; -- dark green
        elsif ((x_int_d / 8) mod 2) /= ((y_int_d / 32) mod 2) then
            grass_rgb <= x"694"; -- medium green
        else
            grass_rgb <= x"8B5"; -- light green
        end if;
    end process;
    ...
    --
    -- Highest priority:
    -- 1. successful whack animation from hit_flash_active
